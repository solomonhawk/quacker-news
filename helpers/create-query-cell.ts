...
interface QueryCellOptions<TData, TError> {
  ...
  isEmpty?: (query: QueryObserverSuccessResult<TData, TError>) => boolean;
}

interface QueryCellOptionsWithEmpty<TData, TError> extends QueryCellOptions<TData, TError> {
  /**
...
interface QueryCellOptionsNoEmpty<TData, TError> extends QueryCellOptions<TData, TError> {
  success: (query: QueryObserverSuccessResult<TData, TError>) => JSXElementOrNull;
}

export function defaultIsEmpty<TData, TError>(query: QueryObserverSuccessResult<TData, TError>): boolean {
  return query.data === null || (Array.isArray(query.data) && query.data.length === 0);
}

export function createQueryCell<TError>(queryCellOpts: CreateQueryCellOptions<TError>) {
  ...
  function QueryCell<TData>(opts: QueryCellOptionsNoEmpty<TData, TError> | QueryCellOptionsWithEmpty<TData, TError>) {
    const { query, isEmpty = defaultIsEmpty } = opts;

    if (query.status === 'success') {
      if ('empty' in opts && isEmpty(query)) {
